...
namespace com.amazon.device.iap.cpt 
{
    public abstract partial class AmazonIapV2Impl
    {
        private abstract class AmazonIapV2Base : AmazonIapV2Impl
        {
            ...
            public override RequestOutput VerifyLicense()
            {
                Start();
                return RequestOutput.CreateFromJson(VerifyLicenseJson("{}"));
            }

            private string VerifyLicenseJson(string jsonMessage)
            {
                Stopwatch stopwatch = new Stopwatch();
                stopwatch.Start();

                string result = NativeVerifyLicenseJson(jsonMessage);
            ...
            protected abstract string NativeVerifyLicenseJson(string jsonMessage);

            public override GetAppstoreSDKModeOutput GetAppstoreSDKMode()
            {
                Start();
                return GetAppstoreSDKModeOutput.CreateFromJson(GetAppstoreSDKModeJson("{}"));
            }

            private string GetAppstoreSDKModeJson(string jsonMessage)
            {
                Stopwatch stopwatch = new Stopwatch();
                stopwatch.Start();

                string result = NativeGetAppstoreSDKModeJson(jsonMessage);
            ...
            protected abstract string NativeGetAppstoreSDKModeJson(string jsonMessage);
            ...
            public override void AddLicenseResponseListener(LicenseResponseDelegate responseDelegate)
            {
                Start();
                string eventId = "licenseResponse";
                lock(eventLock)
                {
                    if (eventListeners.ContainsKey(eventId))
                    {
                        eventListeners[eventId].Add(new LicenseResponseDelegator (responseDelegate));
                    }
                    else 
                    {
                        var list = new List<IDelegator>();
                        list.Add(new LicenseResponseDelegator(responseDelegate));
                        eventListeners.Add(eventId, list);
                    }
                }
            }
            
            public override void RemoveLicenseResponseListener(LicenseResponseDelegate responseDelegate)
            {
                Start();
                string eventId = "licenseResponse";
                lock(eventLock)
                {
                    if (eventListeners.ContainsKey(eventId))
                    {
                        foreach(LicenseResponseDelegator delegator in eventListeners[eventId])
                        {
                            if(delegator.responseDelegate == responseDelegate)
                            {
                                eventListeners[eventId].Remove(delegator);
