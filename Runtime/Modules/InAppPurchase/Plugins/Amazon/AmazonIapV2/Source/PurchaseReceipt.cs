...
namespace com.amazon.device.iap.cpt
{
    public sealed class PurchaseReceipt : Jsonable
    {
        ...
        public string ReceiptId{get;set;}
                ...
                public long DeferredDate{get;set;}
                public string DeferredSku{get;set;}
                public string TermSku{get;set;}
        ...
        public override Dictionary<string, object> GetObjectDictionary() 
        {
            try 
            {
                ...
                objectDictionary.Add("deferredDate", DeferredDate);
                objectDictionary.Add("deferredSku", DeferredSku);
                objectDictionary.Add("termSku", TermSku);
        ...
        public static PurchaseReceipt CreateFromDictionary(Dictionary<string, object> jsonMap) 
        {
            try 
            {
                ...
                if(jsonMap.ContainsKey("deferredDate")) 
                {
                    request.DeferredDate = (long) jsonMap["deferredDate"];
                }
                
                if(jsonMap.ContainsKey("deferredSku")) 
                {
                    request.DeferredSku = (string) jsonMap["deferredSku"];
                }
                
                if(jsonMap.ContainsKey("termSku")) 
                {
                    request.TermSku = (string) jsonMap["termSku"];
                }
