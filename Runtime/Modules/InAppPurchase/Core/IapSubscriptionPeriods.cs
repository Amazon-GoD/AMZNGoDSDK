...
namespace AMZNGoDSDK.Runtime
{
    ...
    internal static class IapSubscriptionPeriods
    {
        ...
        /// действительно ТЕКУЩИЙ («сейчас» лежит внутри него) И чек жив (CancelDate пуст).
        /// У чека, отменённого в прошлом периоде, индекс упирается в потолок CancelDate и
        /// лежит целиком в прошлом — его отсекает проверка индексов. У чека, отменённого
        /// ВНУТРИ текущего периода, индексы ещё совпадают до конца срока — его отсекает
        /// проверка CancelDate: отмена внутри периода — рефанд, начислять нечего. Без этих
        /// проверок каждая чистая установка выдавала бы по периоду за КАЖДЫЙ отменённый чек
        /// истории (логи 2026-08-14: при сроке 1 день два отменённых чека → «+2 периода»;
        /// при сроке 7 дней три чека моложе недели → «+3 периода» — фарм переустановкой).
            ...
            int lastFiredIndex)
        {
            ...
            if (!journalHasEntry)
            {
                ...
                // индексу с потолками (CancelDate/DeferredDate его не срезали). Отменённый
                // чек не засеваем даже при совпадении индексов: отмена внутри текущего
                // периода проходила бы проверку индексов до конца срока (см. summary).
                int byTimeOnly = (int)((trustedNowUtc - FromUnixMs(purchaseDateMs)).TotalDays / termDays);
                if (current == byTimeOnly && cancelDateMs == 0)
                    result.Add(current);
                return result;
            }
